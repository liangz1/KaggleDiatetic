function [database, lenStat, patchNumberToatal, patchNumberPerImage] = CalculateSiftDescriptor(rt_img_dir, rt_data_dir, gridSpacing, patchSize, nrml_threshold)
%==========================================================================

disp('Extracting SIFT features...');
CMVR_folder = dir(rt_img_dir); % CMVR
siftLens = [];
database = [];
...
for ii = 1:length(CMVR_folder)
    subname = CMVR_folder(ii).name; % positive/negative
    %     disp(CMVR_folder(ii).name);
    if ~strcmp(subname, '.DS_Store') && ~strcmp(subname, '.') && ~strcmp(subname, '..')
        ...
        frames = dir(fullfile(rt_img_dir, subname, '*.jpeg'));
        c_num = length(frames);
